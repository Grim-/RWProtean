...
{   
    ...
    public class VerticalTreeStrategy : ITreeDisplayStrategy
    {
            ...
            float spacing)
        {
            ...
            for (int level = 0; level <= maxDepth; level++)
            {
                ...
                var nodesInLevel = levelNodes[level]
                    .Where(n => n.node != null && n.node.type != UpgradeTreeNodeDef.NodeType.Start)
                    .ToList();
                ...
                foreach (var node in nodes.Where(n => n.type != UpgradeTreeNodeDef.NodeType.Start))
                {
        ...
        private List<UpgradeTreeNodeDef> FindStartNodes(List<UpgradeTreeNodeDef> nodes) =>
            nodes.FindAll(n => n.type == UpgradeTreeNodeDef.NodeType.Start);
