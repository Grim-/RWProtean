...
namespace Protean
{
    ...
    public static class EventPatches
    {
        static EventPatches()
        {
            var harmony = new Harmony("com.protean.events");
            harmony.PatchAll();
        }
        ...
        public class HideApparel_Patch
        {
            public static bool Prefix(PawnRenderNode __instance, Pawn pawn, ref Graphic __result)
            {
                if (pawn.health.hediffSet.HasHediff(ProteanDefOf.Protean_SuitActive))
                {
                    ...
                    if (apparel?.def?.apparel?.layers?.Contains(ApparelLayerDefOf.Shell) == true ||
                        apparel?.def?.apparel?.layers?.Contains(ApparelLayerDefOf.Middle) == true)
                    {
                        __result = null;
                        return false;
                    }
